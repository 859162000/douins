...
public class QunarCcicAdapter implements AdapterServiceIFC {
    ...
    private void init(){
        try {
            logger.info("password:"+Configs.get("jsk_password_ccic"));
            logger.info("keypath:"+Configs.get("jks_keystore_path_ccic"));
            logger.info("trustpath:"+Configs.get("jks_truststore_path_ccic"));
            keystoreFile = ResourceUtils.getFile(Configs.get("jks_keystore_path_ccic"));
           
            if(keystoreFile.getPath() != null){     logger.info(keystoreFile.getPath());}else{logger.info("keystore get fail");}
            truststoreFile = ResourceUtils.getFile(Configs.get("jks_truststore_path_ccic"));
            if(truststoreFile.getPath() != null ){     logger.info(truststoreFile.getPath());}else{logger.info("truststore get fail");}
...
//            return StaticTemplate.getExceptionTemplate(templateLoader, DouinsException.getErrorMap(DouinsException.TRADE_TYPE_ERROR));
    ...
    public <T> String doInsure(T data) {
        ...
        //  判断交易类型是否正确
        if(!QunarTransType.CONFIRMMOTORDEALS.getValue().equals(insApplyData.getHeader().getProfileRequest())
                || !QunarTransType.CONFIRMMOTORDEALS.getFunction().equals(insApplyData.getHeader().getFunction())){
//            return StaticTemplate.getExceptionTemplate( templateLoader, DouinsException.getErrorMap(DouinsException.TRADE_TYPE_ERROR));
...
//            List<ProductVo> items = ccicDataService.getProductInfos(ChannelEnum.QUNAR.getCode(), uwData.getInsuredInfoVos().get(i).getProductNo(), saleStrategy);                   // 拉取条款列表
            ...
            uwmain.setStartHour(start.substring(8,10));      // 默认 0 点---暂时修改为去哪返回时间
            uwmain.setEndDate(DateTimeUtils.formateDate(end.substring(0, 8)));       
            uwmain.setEndHour(end.substring(8,10));       // 默认 24 点---暂时修改为去哪返回时间
...
//            insApply.setOriginTradeSerialNo(uw.getTradeNo());
    ...
    private int getDays(String start, String end){
        ...
        try {
            ...
            Date tDate = new Date();
            String  todayDate =format.format(tDate);
            //保费跨天逻辑:1天零54分59秒算1天，1天零55分算2天
            long lim=3299 * 1000; //跨天临界值＝54分59秒   0点临界值
            long lim1=3599 * 1000;//跨天临界值＝59分59秒 10点临界值
            long m=0; //取余
            
            System.out.println(start.substring(0,8));
            System.out.println(todayDate.substring(0, 8));
            if(start.substring(0,8).equals(todayDate.substring(0, 8))){
            	System.out.println("edate=="+eDate.getTime());
            	System.out.println("sdate=="+sDate.getTime());
            	m = (eDate.getTime() - sDate.getTime()) % 86400000;//计算 结束时间－开始时间 的秒差除以一天总秒数的余数
	            	if(m <= lim1){
	            		//如果整除(m=0) 那么时间差就是余数 单位：天
	            		//如果余数小于59分59秒，那么时间差就是余数 单位：天
	            		days = (int)((eDate.getTime() - sDate.getTime()) / 86400000); 
	            	}else{
	            		//如果余数大于lim（59分59秒），则向上取整，多计一天保费 单位：天
    ...
    private ResponseVo verifyPolicyStatus(WithdrawReqVo reqVo){
        ...
        for(PolicyFact p: policys){
            ...
            if(p.getStatus().equals(PolicyStatus.EXPIRED.getValue())){
             ...
             // 查看保单是否生效或过期
                Date now = new Date();
                Date startDate = DateTimeUtils.getDateFromString2(p.getStartDate());
                Date endDate = DateTimeUtils.getDateFromString2(p.getEndDate());
