	...
	private void calPolicyValues(List<PolicyResponseVo> list){
	    if(list!=null&&list.size()>0){
            for(PolicyResponseVo policyResp:list){
            	...
            	if(!PolicyStatus.DRAW_BACK.getValue().equals(policyResp.getStatus())){
            	policyResp.setTotalRevenue(policyResp.getAccuIncome() == null ? new BigDecimal("0.00") : policyResp.getAccuIncome().setScale(2, BigDecimal.ROUND_HALF_UP));
            	policyResp.setYesDayEarn(policyResp.getYesDayIncome() == null ? new BigDecimal("0.00") : policyResp.getYesDayIncome().setScale(2, BigDecimal.ROUND_HALF_UP));
            	}
            	// 可贷金额
