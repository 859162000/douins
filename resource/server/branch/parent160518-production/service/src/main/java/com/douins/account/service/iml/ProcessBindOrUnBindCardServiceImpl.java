...
//		}
	...
	private String getBindOrUnbindCardUrl(UserAccount account,int type) {
		...
		String chanlNo = WebUseConstant.CHANL_NO;//渠道编号
		String baseUrl="";
		String callbackUrl="";
		String flowNo ="";
		if(type==1){
			baseUrl=WebUseConstant.NY_BIND_CARD_URL;//绑定
			callbackUrl=WebUseConstant.NY_BIND_CARD_CALLBACK_URL;//绑卡回调API
			flowNo = account.getBindCardFlowNo();
			
		}else if(type==2){
			baseUrl=WebUseConstant.NY_UNBIND_CARD_URL;// 解绑
			callbackUrl=WebUseConstant.NY_UNBIND_CARD_CALLBACK_URL;//绑卡回调API
		...
		buffer.append(baseUrl).append("?").append("chanlNo=").append(chanlNo).append("&chanlFlowNo=").append(flowNo)
	...
	public ResponseTransVo<BindCardReponseVO> getBindCardInfo(BindCardRequest bindCardRequest) {
		...
		String bankName =bankResponeDao.getBankNameByFrontNumber(creditCard.substring(0,6)) ;
