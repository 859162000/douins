...
public static <T> RequestTransVo<T> readJson2Entity2(String json, Class<T> clazz){
      if(StringUtils.isBlank(json)) return null;
      RequestTransVo<T> temp = new RequestTransVo<T>();
      try {
          T t = null;
          String tString = JSONObject.parseObject(json).getString("requestParams");
          t = objectMapper.readValue(tString, clazz);
          
          temp = objectMapper.readValue(json, temp.getClass());
          temp.setRequestParams(t);
     } catch (JsonParseException e) {
         e.printStackTrace();
     } catch (JsonMappingException e) {
         e.printStackTrace();
