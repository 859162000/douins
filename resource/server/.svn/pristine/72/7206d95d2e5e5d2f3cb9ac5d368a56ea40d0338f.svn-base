...
public class PolicyManageServiceImp implements PolicyManageService{
	...
	public PolicyResponse processUnderwrite(PolicyRequest policyRequest) {
		...
		try {
			...
			if(ResponseCode.SUCCESS.getValue().equals(resTrans.getResponseCode())){
               ...
               logger.info("trasBusinessService============"+trasBusinessService);
	...
	private BusinessTrans creatBusinessTrans(PolicyRequest policyRequest,String transId, String orderNumber) {
         ...
         if(policyVo!=null){
        	 logger.info("policyVo is not null");
        	 businessTrans.setBusinessId(policyVo.getInsuredId());
        	 ...
        	 businessTrans.setPayMoney(policyVo.getRepayAmount());
        	 ...
        	 businessTrans.setTransUserToken(policyVo.getUserId());
        	 //businessTrans.setTransChannelNo(policyRequest.getPolicyVo().);
        	 //businessTrans
        	 //购买保单时候把保单id传来
        	 businessTrans.setTransInsureId(policyVo.getProductId());
        	 businessTrans.setTransProductName(policyVo.getProductName());
         }
