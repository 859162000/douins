...
public class PolicyResponse extends ResponseTransVo{
    ...
    private BigDecimal calTotalAmount(){
        ...
        if(policyList != null){
            for(PolicyResponseVo vo: policyList){
            	if(!PolicyStatus.DRAW_BACK.getValue().equals(vo.getStatus())){
                total = total.add(vo.getTotalPrem()).setScale(2, BigDecimal.ROUND_HALF_UP);//总保费
            	}
            }
        }
    ...
    private BigDecimal caltotalRevenue() {
    	...
    	if(policyList != null){
    		for(PolicyResponseVo vo: policyList){
    			if(!PolicyStatus.DRAW_BACK.getValue().equals(vo.getStatus())){
    			total = total.add(vo.getAccuIncome()==null?new BigDecimal(0):vo.getAccuIncome()).setScale(2, BigDecimal.ROUND_HALF_UP);//累计总收益 对应数据库中 T_policy_master中的 ACCUMULATED_INCOME （累计收益）
    			}
    		}
    	}
    ...
    private BigDecimal TotalYesterdayIncome() {
    	...
    	if(policyList != null){
    		for(PolicyResponseVo vo: policyList){
    			if(!PolicyStatus.DRAW_BACK.getValue().equals(vo.getStatus())){
    			total = total.add(vo.getYesDayIncome()==null?new BigDecimal(0):vo.getYesDayIncome()).setScale(2, BigDecimal.ROUND_HALF_UP);//昨日收益
    		}}
    	}
